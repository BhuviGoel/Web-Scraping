...
browser = webdriver.Chrome(r"C:\Users\bhuvi\Google Drive\Coding\Python\C127 - Web Scraping\chromedriver.exe")
...
for index, data in enumerate(planet_data):
    scrape_more_data(data[5]) #scrapes data
final_planet_data = []
for index, data in enumerate(planet_data):
    new_planet_data_element = new_planet_data[index]
    new_planet_data_element = [elem.replace("\n", "") for elem in new_planet_data_element]
    new_planet_data_element = new_planet_data_element[:7]
    final_planet_data.append(data + new_planet_data_element) #merges data
